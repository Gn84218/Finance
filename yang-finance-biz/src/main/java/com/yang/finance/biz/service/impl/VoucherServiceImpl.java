...
import com.yang.finance.biz.constant.MqConstant;
import com.yang.finance.biz.constant.RedisKeyConstant;
import com.yang.finance.biz.domain.*;
...
import com.yang.finance.biz.enums.ListVoucherSortFieldEnum;
import com.yang.finance.biz.enums.MqMsgStatusEnum;
import com.yang.finance.biz.mapper.VoucherMapper;
import com.yang.finance.biz.mapper.VoucherSubjectAssistCalculateDetailMapper;
...
import static com.yang.finance.biz.domain.VoucherField.*;
    ...
    public boolean save(CreateVoucherForm form) {
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(form.getVoucherSubjectAssistCalculateDetailFormList())) {
                //统计前端传递的所有辅助核算id列表(查询前端所传递的辅助核算明细)
            ...
            for (CreateVoucherForm.VoucherSubjectDetailForm voucherSubjectDetailForm : form.getVoucherSubjectDetailFormList()) {
                //过滤出数据库与前端传递 匹配的id
                GetSubjectVo getSubjectVo = getSubjectVoList.stream().filter(p -> p.getId().equals(voucherSubjectDetailForm.getSubjectId()))
                        .findFirst()
                        .orElse(null);
                ...
                if (getSubjectVo.getSubjectCalculateConfigVo().getEnableAssistCalculateConfigs()) {
                    ...
                    getSubjectVo.getSubjectCalculateConfigVo().getAssistCalculateConfigs().forEach(p -> {
                        ...
                        List<CreateVoucherForm.VoucherSubjectAssistCalculateDetailForm> voucherSubjectAssistCalculateDetailForms
                                = form.getVoucherSubjectAssistCalculateDetailFormList().stream()
                                .filter(m -> Objects.equals(voucherSubjectDetailForm.getRowNo(), m.getRowNo())
                                        && m.getSubjectId().equals(getSubjectVo.getId())
                                        && m.getAssistCalculateCateId().equals(p.getAssistCalculateId()))
            ...
            //获取凭证不为空,历遍取出所有凭证id 为空就-1 否则根据id的使用次数(count)做-1
